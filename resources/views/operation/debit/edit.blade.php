        ...
        <div class="row mb-5">
            <div class="col-lg-9 col-12 mx-auto">
                <div class="card card-body mt-4">
                    <h6 class="mb-0">Editar Débito</h6>
                    <hr class="horizontal dark my-3">
                    <form method="POST" action="{{ route('debit-edit.update', $debit->id) }}" enctype="multipart/form-data">
                        @csrf
                    <div class="px-4" id="alert">
                        ...
                        <div class="mb-3">
                            <select class="form-select form-control" id="choices-reservation" name="reservation_id">
                                ...
                                @foreach ($reservations as $reservation)
                                    <option value="{{ $reservation->id }}" {{ $reservation->id == old('reservation_id',$debit->reservation_id) ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-select form-control" id="choices-type" name="type">
                                ...
                                <option value="Estacionamento" {{ $debit->type === 'Estacionamento' ? 'selected' : '' }}>Estacionamento</option>
                                <option value="Pedagio" {{ $debit->type === 'Pedagio' ? 'selected' : '' }}>Pedágio</option>
                            </select> 
                            @error('type')
                            <p class='text-danger text-xs pt-1'> {{ $message }} </p>
                            @enderror
                        ...
                        <div class="form-group">
                            <label class="number"> Valor</label>
                            <input type="number" step="0.001" class="form-control" id="amount" name="amount" value="{{ old('amount', $debit->amount) }}" >
                            @error('amount')
                            <p class='text-danger text-xs pt-1'> {{ $message }} </p>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="description" id="description" class="form-control"> {{ old('description', $debit->description) }}</textarea>
                            @error('description')
                            <p class='text-danger text-xs pt-1'> {{ $message }} </p>
                            @enderror
                        </div>
